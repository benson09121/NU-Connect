...
export async function getVenuesWithAvailability(
  ...
  const parseTime = (d: Date): number => {
    // Prisma returns time fields as Date with a 1970-01-01 base
    return d.getTime();
  };
  ...
  return venues.map((v) => {
    ...
    if (start_date && start_time && end_time) {
      ...
      for (const ev of v.tbl_event_venue) {
        const e = ev.tbl_event;
        if (!activeStatuses.includes(e.status as string)) continue;
        if (exclude_event_id && e.event_id === exclude_event_id) continue;

        // Date overlap
        const eDateStart = new Date(e.start_date);
        const eDateEnd = new Date(e.end_date);
        if (eDateStart > sdEnd || eDateEnd < sdStart) continue;

        // Time overlap
        const eTimeStart = parseTime(e.start_time);
        const eTimeEnd = parseTime(e.end_time);
        ...
        conflictingEvents.push({
          ...
          start_time: e.start_time.toISOString().slice(11, 19),
          end_time: e.end_time.toISOString().slice(11, 19),
        });
      }
    }
