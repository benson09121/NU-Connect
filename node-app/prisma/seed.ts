...
async function main() {
  ...
  //    SQL perm ids: 1,9,16,11,12,13,14,23,4,24,25,17,19,20,21,22,29,31,34,35,36,37,38,39,40,41,42
  // -------------------------------------------------------------------------
  ...
  const userSDAlevel2 = await prisma.tbl_user.upsert({
    where: { email: 'sdao.director@nu.edu.ph' },
    update: {},
    create: {
      f_name: 'Maria',
      l_name: 'Santos',
      email: 'sdao.director@nu.edu.ph',
  ...
  const userAdviser1 = await prisma.tbl_user.upsert({
    where: { email: 'adviser.jpcs@nu.edu.ph' },
    update: {},
    create: {
      f_name: 'Ricardo',
      l_name: 'Lim',
      email: 'adviser.jpcs@nu.edu.ph',
      program_id: programs['BSIT-MWA'],
  ...
  // -------------------------------------------------------------------------
  // 21. ORGANIZATIONS  (5 orgs × 1 000 members + 5 committees each)
  // -------------------------------------------------------------------------
      ...
      for (const uid of paidIds) {
        const txn = await prisma.tbl_transaction.create({
          data: {
            ...
            transaction_type_id: txnTypes['MEMBERSHIP'],
  ...
  if (existingEvents === 0) {
    ...
    for (const orgRow of createdOrgs) {
      ...
      for (const tplIdx of chosenTemplates) {
        ...
        const startTime = new Date(`1970-01-01T${tpl.startTime}`);
        const endTime   = new Date(`1970-01-01T${tpl.endTime}`);

        const imageFilename = `${faker.string.uuid()}.jpg`;

        await prisma.tbl_event.create({
          data: {
  ...
  // -------------------------------------------------------------------------
  // 23. EVENT ATTENDANCE & EVALUATIONS
  // -------------------------------------------------------------------------
  ...
  if (existingAttendance === 0) {
    ...
    for (const event of allEvents) {
      ...
      const attendanceStatus = isPast ? 'Attended' : 'Registered';
      ...
      // ~70% of past attendees submit an evaluation
      faker.seed(event.event_id * 13); // deterministic per event
      const evaluators = members.filter(() => faker.number.int({ min: 1, max: 10 }) <= 7);
